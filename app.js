...
const specialPattern = /Marvel One-Shot|The Consultant|A Funny Thing|Item 47|Team Thor|Team Darryl|Peter's To-Do List|Agents of S\.H\.I\.E\.L\.D\.: Slingshot|Werewolf by Night|GOTG Holiday Special|One Last Kill/;
const isSeriesTitle = title => seriesPattern.test(title) && !specialPattern.test(title);
const getEpisodeCount = title => { if (title.startsWith('Agents of S.H.I.E.L.D.')) return title.includes('Season 6 & 7') ? 26 : 22; const key = Object.keys(episodeCounts).find(candidate => title.startsWith(candidate)); return key ? episodeCounts[key] : 8; };
const getDisplayType = title => specialPattern.test(title) ? 'Special' : seriesPattern.test(title) ? 'Series' : 'Movie';
const items = rawOrder.split('\n').map((title,index) => ({ id:`${index}-${title}`, title:title.trim(), index, type:isSeriesTitle(title) ? 'series' : 'movie', displayType:getDisplayType(title), episodes:isSeriesTitle(title) ? getEpisodeCount(title) : 0 }));
